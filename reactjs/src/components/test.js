...
export default function Home() {
  const [renderAdditionalSelect, setRenderAdditionalSelect] = useState('');
  ...
  const fetchInfo = async () => {
    return await fetch("http://127.0.0.1:8000/api/gestiono")
        .then((response) => response.json())
  ...
  useEffect(() => {
    let filteredUsers = [];
    if (mailState === "recu") {
      filteredUsers = lists.filter((user) => user.status === "1" && (
      ...
      filteredUsers = lists.filter((user) => user.status === "0" && (
          user.sujet.toLowerCase().includes(searchInput.toLowerCase()) ||
          user.employe.toLowerCase().includes(searchInput.toLowerCase())
      ));
    } else {
      filteredUsers = lists.filter((user) =>
          user.sujet.toLowerCase().includes(searchInput.toLowerCase()) ||
          user.employe.toLowerCase().includes(searchInput.toLowerCase())
      );
    }
    setFilteredlists(filteredUsers);
  ...
  const handleChangeselect = (e) => {
    ...
    if (selectedValue === "travail effectué") {
      console.log("tr")
      setRenderAdditionalSelect(true);
    } else {
      setRenderAdditionalSelect(false);
...
<>
      ...
      <div className="modal-content">
        ...
        <div className="modal-body">
          <form onSubmit={handleSubmit} encType="multipart/form-data" id="imageForm">
            ...
            <div className="mb-3">
              <label htmlFor="date" className="form-label">type courriel</label>
              <select
                  className="form-select"
                  size="2"
                  id="type_courriel"
                  aria-label="size 2 select example"
                  onChange={handleChangeform} onClick={handleChangeselect}>
                <option value="archive">archive</option>
                <option value="travail effectué">travail effectué</option>
              </select>
              {renderAdditionalSelect && (<div>
                    <div className="mb-3">
                      <label htmlFor="date" className="form-label">Date de commission</label>
                      <input type="date" className="form-control" id="datec" aria-describedby="emailHelp" onChange={handleChangeform} />
                    </div><div className="mb-3">
                    <label htmlFor="date" className="form-label">La date précisée</label>
                    <input type="date" className="form-control" id="datep" aria-describedby="emailHelp" onChange={handleChangeform} />
                  </div>
            ...
            <div className="modal-footer">
              <PurpleOutlineButton dataDismiss={"modal"} type={"reset"}>Close</PurpleOutlineButton>
              <PurpleOutlineButton dataDismiss={"modal"} type={"submit"} >Save change </PurpleOutlineButton>
            </div>
      ...
      <table>
        ...
        {currentUsers.map((user, index) => (
            <tr key={index}>
              ...
              <td>{user.intérêt} </td>
              <td>{user.employe}</td>
              <td className="text-center"><label className="badge rounded-pill text-bg-dark "></label>{user.type_courriel}
              </td>
              ...
              <td>{user.status === "0" ? (
                      <>
                        <span className="badge text-bg-danger">en attendant</span>
                      </>
                  ) :
                  (<span className="center badge text-bg-success">reçu</span>)
              }
              </td>
              {userdetail.role === 1 ? (<>
                  ...
                  <div className="modal-content">
                    ...
                    <div className="modal-body">
                      <form onSubmit={hundleUpdateSubmit} encType="multipart/form-data" id="imageForm">
                        ...
                        <div className="modal-footer">
                          <PurpleOutlineButton dataDismiss={"modal"} type={"reset"}>Close</PurpleOutlineButton>
                          <PurpleOutlineButton type={"submit"} dataDismiss={"modal"}>Save change </PurpleOutlineButton>
                        </div>
                      </form>
                    </div>
                  </div>
  ...
  <div className="m-2 d-grid gap-2 d-md-block">
    ...
    <PurpleOutlineButton onClick={() => setCurrentPage(currentPage + 1)}
                         disabled={currentPage === Math.ceil(lists.length / usersPerPage)}>
      Next
    </PurpleOutlineButton>
  </div>
</>
